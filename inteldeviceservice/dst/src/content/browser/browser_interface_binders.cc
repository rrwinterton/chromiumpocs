...
DeviceServiceProviderBinder& GetDeviceServiceProviderBinderOverride() {
  static base::NoDestructor<DeviceServiceProviderBinder> binder;
  return *binder;
}
...
void PopulateFrameBinders(RenderFrameHostImpl* host, mojo::BinderMap* map) {
  ...
  map->Add<device::mojom::DeviceServiceProvider>(
      base::BindRepeating(&BindDeviceServiceProvider));
...
void OverrideDeviceServiceProviderBinderForTesting(
    DeviceServiceProviderBinder binder) {
  internal::GetDeviceServiceProviderBinderOverride() = std::move(binder);
}
