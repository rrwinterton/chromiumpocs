...
void LocalFrameMojoHandler::Trace(Visitor* visitor) const {
  ...
  visitor->Trace(device_service_provider_service_);
  ...
  visitor->Trace(device_service_receiver_);
}
...
device::mojom::blink::DeviceServiceType
LocalFrameMojoHandler::GetDeviceService() {
  if (device_service_provider_service_.is_bound())
    return current_device_service_;

  auto task_runner = frame_->GetTaskRunner(TaskType::kInternalDefault);
  frame_->GetBrowserInterfaceBroker().GetInterface(
      device_service_provider_service_.BindNewPipeAndPassReceiver(task_runner));

  device_service_provider_service_->AddListenerAndGetCurrentService(
      device_service_receiver_.BindNewPipeAndPassRemote(task_runner),
      WTF::BindOnce(&LocalFrameMojoHandler::OnServiceChanged,
                    WrapPersistent(this)));
  return current_device_service_;
}
...
void LocalFrameMojoHandler::OnServiceChanged(
    device::mojom::blink::DeviceServiceType service) {
  if (!RuntimeEnabledFeatures::DeviceServiceEnabled())
    return;
  current_device_service_ = service;
  // A change of the device service requires re-evaluation of media queries
  // for the local frame subtree (the device service affect the
