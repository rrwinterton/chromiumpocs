...
MediaValuesCached::MediaValuesCachedData::MediaValuesCachedData(
  ...
  if (frame && frame->View()) {
    ...
    device_service = MediaValues::CalculateDeviceService(frame);
  }
}
...
device::mojom::blink::DeviceServiceType MediaValuesCached::GetDeviceService()
    const {
  return data_.device_service;
}
