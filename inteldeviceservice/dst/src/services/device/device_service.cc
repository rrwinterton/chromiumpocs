...
#if BUILDFLAG(IS_WIN)
void DeviceService::BindDeviceServiceProvider(
    mojo::PendingReceiver<mojom::DeviceServiceProvider> receiver) {
  if (!device_service_provider_) {
    auto service_platform_provider_ = DeviceServicePlatformProvider::Create();
    if (!service_platform_provider_)
      return;
    device_service_provider_ = std::make_unique<DeviceServiceProviderImpl>(
        std::move(service_platform_provider_));
  }
  device_service_provider_->Bind(std::move(receiver));
}
#endif
