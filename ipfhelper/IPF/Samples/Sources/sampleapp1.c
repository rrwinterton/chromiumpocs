...
#define PROGRAM_NAME			"sampleapp1"
#define PROGRAM_DESCRIPTION		"IPF Core API Sample Application"
#define	PROGRAM_VERSION			IPF_APP_VERSION
#define PROGRAM_SERVERADDR		PUBLIC_SERVERADDR   // Use PUBLIC_SERVERADDR for operations requiring only read permission
...
// Dynamic Tuning Technology to obtain state information and events related to
...
static esif_error_t ExerciseInterface(IpfSession_t session)
{
	...
	//
	// Loop displaying the processor temperature and power
	//
	if (ESIF_OK == rc) {
...
)
{
	...
	for (int i = 1; i < argc; i++) {
		...
		if ((curOpt = ipf_os_strpbrk(curArg, ":=")) != NULL) {
